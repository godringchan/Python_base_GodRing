    ...
    def createCalculator(self):
        ...
        for row_index, r_value in enumerate(btnText):
            ...
            for column_index, one_value in  enumerate(r_value):
                ...
                btn.grid(row=(row_index+1), column=column_index, pady=5, padx=1,  sticky="NSEW")
                if one_value == "=":
                    btn.grid(row=(row_index+1), column=column_index, pady=5, padx=1,  sticky="NSEW", rowspan=2)
                if one_value == 0:
                    btn.grid(row=(row_index+1), column=column_index, pady=5, padx=1,  sticky="NSEW", columnspan=2)
                if one_value == ".":
                    btn.grid(row=(row_index+1), column=column_index+1, pady=5, padx=1,  sticky="NSEW")
